...
export default class VerificationA extends Component {
  ...
  submit = async () => {
    ...
    try {
      const confirmSignup = await Auth.confirmSignUp(this.props.store.authStore.username, this.state.pin);
      confirmSignup.then((res) => {
        console.log('res', res)
        this.closeModal();
        this.navigateTo('HomeNavigator');
      })
  ...
  render() {
    ...
    console.log('this.props.store.authStore.username', this.props.store.authStore.username)
    return (
